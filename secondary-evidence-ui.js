...
function imagingCriterionGroupId(criterionId) {
  return criterionId.match(/^((?:BONE|DISC)-3[ab]|JNT-3d)-(?:definitive|equivocal)$/)?.[1];
}
function renderImagingCriterionGroup(criteria, evidence, evaluation, openCriteria, defaultOpenCriterion) {
  const groupId = imagingCriterionGroupId(criteria[0].id);
  const criterionNumber = groupId.split("-")[1];
...
export function renderCompactMenEvidence({ definition, evaluation, patientAge, evidence = {}, openCriterion = "", openCriteria }) {
  ...
  const hasImagingCriterionGroups = ["BONE", "DISC", "JNT"].includes(definition.siteCode);
  const criterionMarkup = hasImagingCriterionGroups
    ? [...criteria.reduce((groups, criterion) => {
      const groupedId = imagingCriterionGroupId(criterion.id);
