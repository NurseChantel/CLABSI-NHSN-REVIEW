...
function renderSiteGuide() {
 ...
 container.querySelectorAll("input[data-evidence-id]").forEach(input => input.addEventListener("change", () => { state.openMenCriterion = usesCriterionCenteredBjReview ? input.closest("details[data-men-criterion]")?.dataset.menCriterion || "" : ""; state.siteEvidence[input.dataset.evidenceId] = checkboxEvidenceValue(input.checked); updateAll(); }));
 ...
 container.querySelectorAll("details[data-men-criterion]").forEach(details => details.addEventListener("toggle", () => { if (details.open) { if (usesCriterionCenteredBjReview) container.querySelectorAll("details[data-men-criterion]").forEach(other => { if (other !== details) other.open = false; }); state.openMenCriterion = details.dataset.menCriterion; } else if (state.openMenCriterion === details.dataset.menCriterion) state.openMenCriterion = ""; }));
}
