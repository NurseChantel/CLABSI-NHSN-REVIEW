...
function renderSiteGuide() {
 ...
 const usesEvidenceReview = ["MEN", "SA"].includes(definition?.siteCode);
...
function getSiteSpecificDefinitionStatus() { const evaluation = getSecondaryEvaluation(); if (evaluation.status === "siteNotSelected") return { status: "incomplete", met: false, label: "Site-specific definition incomplete", reason: state.selectedMajorCategory ? "Select a specific NHSN site code; the major category cannot qualify." : "Select a major category and then a specific NHSN site code." }; if (evaluation.status === "siteNotValidated") return { status: "incomplete", met: false, label: "Source validation required", reason: placeholderWarning }; if (evaluation.siteDefinitionMet) return { status: "met", met: true, label: state.selectedSite === "SA" ? "SA site definition met" : "MEN site definition met", reason: `${evaluation.metCriterion} is completely satisfied.` }; return { status: "incomplete", met: false, label: evaluation.status === "exclusionApplies" ? "Cannot qualify because an exclusion applies" : evaluation.status === "notStarted" ? "Not started" : "Incomplete", reason: evaluation.status === "exclusionApplies" ? "A documented other recognized cause prevents an asterisked finding from qualifying." : state.selectedSite === "SA" ? "Complete one SA criterion branch without combining NHSN branches." : "Complete one MEN criterion branch without combining incompatible finding groups." }; }
function determineSecondaryStatus() { const evaluation = getSecondaryEvaluation(); return { met: evaluation.secondaryAttributionMet, status: evaluation.secondaryAttributionMet ? "met" : evaluation.siteDefinitionMet && evaluation.attributionMissing?.some(item => item.includes("not met")) ? "notMet" : "incomplete", architectureStatus: evaluation.status, label: evaluation.secondaryAttributionMet ? "Secondary BSI attribution met" : "Secondary BSI review incomplete", reason: evaluation.secondaryAttributionMet ? `${state.selectedSite === "SA" ? "SA" : "MEN"} and both authorized attribution requirements are met.` : evaluation.siteDefinitionMet ? evaluation.attributionMissing.join("; ") : getSiteSpecificDefinitionStatus().reason }; }
...
function renderCalculatedStatuses() {
  ...
  const attributionUnlocked = !["MEN", "SA"].includes(state.selectedSite) || site.met;
