...
test("DISC 3a requires a finding, blood organism, and imaging", () => {
  ...
  assert.equal(evaluate({ ...definitive, "disc-blood-organism": "notMet" }).metCriterion, "DISC-3b");
...
test("DISC 3b requires a finding and imaging", () => {
  assert.equal(evaluate({ "disc-pain": "met", "disc-definitive-imaging": "met" }).metCriterion, "DISC-3b");
...
test("equivocal DISC imaging requires documented antimicrobial treatment", () => {
  assert.equal(evaluate({ "disc-fever": "met", "disc-equivocal-imaging": "met", "disc-antimicrobial-treatment": "met" }).metCriterion, "DISC-3b");
...
test("the other-recognized-cause exclusion applies only to DISC pain", () => {
  ...
  assert.equal(evaluate({ "disc-fever": "met", "disc-definitive-imaging": "met", "other-recognized-cause": "met" }).metCriterion, "DISC-3b");
});

test("DISC has no age-specific boundary and accepts either listed finding", () => {
  assert.equal(evaluate({ "disc-fever": "met", "disc-definitive-imaging": "met" }, { patientAge: "infant" }).metCriterion, "DISC-3b");
  assert.equal(evaluate({ "disc-pain": "met", "disc-definitive-imaging": "met" }, { patientAge: "adult" }).metCriterion, "DISC-3b");
});
