...
function statusHtml(subtype, result) {
  ...
  const stillNeeded = needed.length ? `Still needed: ${esc(needed.join(", "))}` : (met ? "Imaging, clinical findings, and laboratory findings satisfied" : "Complete dated patient and imaging fields to begin.");
  return `<div class="pneu-status-strip ${met ? "met" : "warning"}" role="status"><strong>${subtype} — ${met ? "MET" : "NOT MET"}</strong><span>${stillNeeded}</span>${met ? `<span>DOE <b>${esc(evaluation.dateOfEvent || "—")}</b> · IWP <b>${esc(evaluation.infectionWindow?.start || "—")} – ${esc(evaluation.infectionWindow?.end || "—")}</b></span>` : ""}</div>`;
}
...
function imagingSection(input, evaluation, errors, source) {
  ...
  const rows = input.imagingStudies.map((study, i) => `<div class="pneu-study-row" data-image-row="${i}">${field("Date", `imagingStudies.${i}.date`, study.date, "date", "", errorFor(errors, `image-date-${i}`))}<label class="pneu-field"><span>Finding</span><select data-image-select data-index="${i}"><option value="">Select</option>${[...imageFindings, ...(infant ? [["pneumatoceles", "Pneumatoceles"]] : [])].map(([value, label]) => `<option value="${value}"${selected(study.findings[0], value)}>${label}</option>`).join("")}</select></label><label class="pneu-field"><span>Interpretation</span><select data-pneu-field="imagingStudies.${i}.interpretation"><option value="definitive"${selected(study.interpretation, "definitive")}>Definitive</option><option value="equivocal"${selected(study.interpretation, "equivocal")}>Equivocal</option></select></label>${i > 0 ? `<label class="pneu-field"><span>Pattern</span><select data-pneu-field="imagingStudies.${i}.relationshipType"><option value="persistent"${selected(study.relationshipType, "persistent")}>Persistent</option><option value="progressive"${selected(study.relationshipType, "progressive")}>Progressive</option></select></label>` : ""}${input.imagingStudies.length > 1 ? `<button type="button" class="link-button" data-pneu-remove="imagingStudies" data-index="${i}">Remove</button>` : ""}<label><input type="checkbox" data-pneu-field="imagingStudies.${i}.attributedToOtherCondition"${checked(study.attributedToOtherCondition)}> Other cause</label>${study.interpretation === "equivocal" ? `<label><input type="checkbox" data-pneu-field="imagingStudies.${i}.clinicalCorrelation"${checked(study.clinicalCorrelation)}> Correlated</label>` : ""}</div>`).join("");
  const imageMet = evaluation?.imaging?.met;
  return `<div class="pneu-compact-section"><div class="pneu-cell-title">Required study pattern <span class="criterion-chip ${imageMet ? "met" : ""}">${imageMet ? "Met" : "Needed"}</span></div><div class="pneu-compact-checks"><label><input type="checkbox" disabled${checked(input.imagingStudies.length >= 2)}> Two or more serial studies</label><label><input type="checkbox" data-pneu-field="soleAvailableImage"${checked(input.soleAvailableImage)}> Qualifying one-study exception</label></div><fieldset class="pneu-choice"><legend>Underlying pulmonary/cardiac disease?</legend><label><input type="radio" name="underlying" data-pneu-field="underlyingPulmonaryOrCardiacDisease" value="true"${checked(input.underlyingPulmonaryOrCardiacDisease)}> Yes</label><label><input type="radio" name="underlying" data-pneu-field="underlyingPulmonaryOrCardiacDisease" value="false"${checked(!input.underlyingPulmonaryOrCardiacDisease)}> No</label></fieldset><div class="pneu-cell-title">Studies</div>${rows}<button type="button" data-pneu-add="imagingStudies">+ Add study</button><details class="pneu-guidance"><summary>View NHSN footnotes</summary><p>${esc(source.document)}, ${esc(source.section)}, pages ${esc(source.printedPage)}.</p></details></div>`;
}
...
function bulletLabel(number) { return `Bullet ${String.fromCharCode(64 + number)}`; }
function respiratoryGroup(input, number, kinds, extra = "") { const count = groupMet(input, kinds); return `<fieldset class="pneu-bullet-group ${count ? "selected" : ""}"><legend>${bulletLabel(number)} ${count ? "✓" : ""}</legend>${kinds.filter(k => k !== "tachypnea").map(kind => findingOption(input, kind)).join("")}${kinds.includes("tachypnea") ? measurementControl(input, "respiratory-rate", "Tachypnea", "age-specific threshold") : ""}${extra}</fieldset>`; }
...
function sourceDetails(source) { return `<details class="pneu-guidance"><summary>View NHSN footnotes</summary><p>${esc(source.document)} — ${esc(source.section)}; ${esc(source.printedPage)}.</p></details>`; }
function pathwayBanner(text) { return `<p class="pneu-or-intro pneu-pathway-banner">${esc(text)}</p>`; }
function criterionTable(className, headers, cells) { return `<div class="pneu-table-scroll"><div class="pneu-criteria-table ${className}">${headers.map(h => `<div class="pneu-table-header">${esc(h)}</div>`).join("")}${cells.map(cell => `<div class="pneu-table-cell ${cell.className}">${cell.html}</div>`).join("")}</div></div>`; }

function pnu1Criteria(state, input, evaluation, errors) {
  const age = ageYears(input);
  const branch = age !== null && age <= 1 ? { title: "PNU1 — Infant ≤1 year", clinical: infantClinical(input) } : age !== null && age > 1 && age <= 12 ? { title: "PNU1 — Child >1 through ≤12 years", clinical: childClinical(input) } : { title: "PNU1 — Any patient", clinical: anyClinical(input) };
  return `${pathwayBanner(`Active age pathway: ${branch.title}`)}${criterionTable("pnu1", ["Imaging Test Evidence", "Signs / Symptoms"], [{ className: "pneu-imaging-cell", html: imagingSection(input, evaluation, errors, PNU1_SOURCES.imaging) }, { className: "pneu-symptoms-cell", html: `<div class="pneu-cell-title">${esc(branch.title)}</div>${branch.clinical}` }])}<aside class="pneu-note">PNU1 has no laboratory criterion and does not independently report a pathogen. ${sourceDetails(PNU1_SOURCES.checklist)}</aside>`;
}
...
function labSelector(state, input, errors, algorithm) { const active = state.selectedLabAlternative[algorithm]; return `<section class="pneu-checklist-block"><h5>${algorithm === "common" ? "Common bacterial or filamentous fungal pathogens — select at least ONE qualifying laboratory pathway" : "Viral, Legionella, and other bacterial pneumonias — select at least ONE qualifying definitive laboratory finding"}</h5><div class="pneu-lab-options">${labOptions[algorithm].map(([id, label]) => `<label><input type="radio" name="lab-${algorithm}" data-lab-alternative="${id}" data-algorithm="${algorithm}"${checked(active === id)}> ${label}</label>`).join("")}</div>${labRecordFields(input, errors, algorithm, active)}</section>`; }
function pnu2Criteria(state, input, evaluation, errors) {
  const labHtml = `${labSelector(state, input, errors, "common")}<div class="pneu-or">OR</div>${labSelector(state, input, errors, "definitive")}${sourceDetails(PNU2_SOURCES.common)}${sourceDetails(PNU2_SOURCES.other)}`;
  return `${pathwayBanner("Active pathway: PNU2. Meeting either complete PNU2 algorithm qualifies PNU2.")}${criterionTable("pnu2", ["Imaging Test Evidence", "Signs / Symptoms", "Laboratory"], [{ className: "pneu-imaging-cell", html: imagingSection(input, evaluation, errors, PNU2_SOURCES.imaging) }, { className: "pneu-symptoms-cell", html: anyClinical(input, true) }, { className: "pneu-lab-cell", html: labHtml }])}`;
}
...
function pnu3Criteria(state, input, evaluation, errors) {
  return `${pathwayBanner("Active pathway: PNU3 — immunocompromised patient")}${criterionTable("pnu3", ["Imaging Test Evidence", "Signs / Symptoms", "Laboratory"], [{ className: "pneu-imaging-cell", html: imagingSection(input, evaluation, errors, PNU3_SOURCES.imaging) }, { className: "pneu-symptoms-cell", html: `${hostFields(state, input)}${and()}${pnu3Clinical(input)}` }, { className: "pneu-lab-cell", html: pnu3Laboratory(state, input, errors) }])}`;
}
