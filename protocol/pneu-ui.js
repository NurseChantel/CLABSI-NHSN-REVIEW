...
function columnStatus(met, started) { const label = met ? "Met" : started ? "In progress" : "Needed"; return `<span class="pneu-column-status ${met ? "met" : started ? "progress" : ""}">${label}</span>`; }
function manualColumn(title, status, body) { return `<section class="pneu-manual-column"><header class="pneu-column-heading"><h4>${esc(title)}</h4>${status}</header><div class="pneu-column-body">${body}</div></section>`; }
function manualTable(columns, subtype) { return `<div class="pneu-manual-table ${subtype.toLowerCase()}" role="group" aria-label="${subtype} NHSN criteria table">${columns.join('<div class="pneu-table-and" aria-hidden="true">AND</div>')}</div>`; }
function criterionPanel(id, title, body, source, met = false) { return `<section class="pneu-main-criterion" data-pneu-criterion="${id}"><header class="pneu-criterion-heading"><span>${esc(title)}</span><span class="criterion-chip ${met ? "met" : ""}">${met ? "Criterion met" : "Complete all columns"}</span></header><div class="pneu-criterion-body">${body}${sourceDetails(source)}</div></section>`; }
...
function pnu1Criteria(state, input, evaluation, errors) {
  ...
  const imageStarted = input.imagingStudies.some(study => study.date || study.findings.length); const clinicalStarted = input.measurements.length > 0 || input.clinicalFindings.length > 0;
  return `<p class="pneu-or-intro">Meeting any one complete PNU1 row qualifies PNU1.</p>${branches.map(branch => {
    const branchMet = evaluation?.met && evaluation.clinical.branch === branch.id;
    const table = manualTable([manualColumn("Imaging evidence", columnStatus(evaluation?.imaging?.met, imageStarted), imagingSection(input, evaluation, errors, PNU1_SOURCES.imaging)), manualColumn("Signs and symptoms", columnStatus(branchMet, clinicalStarted), branch.clinical())], "PNU1");
    return criterionPanel(branch.id, branch.title, table, PNU1_SOURCES.checklist, branchMet);
  }).join('<div class="pneu-or">OR</div>')}<aside class="pneu-note">PNU1 has no laboratory criterion and does not independently report a pathogen.</aside>`;
}
...
function pnu2Criteria(state, input, evaluation, errors) {
  const metBranch = evaluation?.laboratoryEvidence?.branches?.find(x => x.met)?.id; const metAlgorithm = metBranch && DEFINITIVE_BRANCHES.has(metBranch) ? "definitive" : metBranch ? "common" : "";
  const imageStarted = input.imagingStudies.some(study => study.date || study.findings.length); const clinicalStarted = input.measurements.length > 0 || input.clinicalFindings.length > 0; const labStarted = input.microbiologyResults.length > 0 || input.histopathologyResults.length > 0;
  const table = algorithm => manualTable([manualColumn("Imaging evidence", columnStatus(evaluation?.imaging?.met, imageStarted), imagingSection(input, evaluation, errors, PNU2_SOURCES.imaging)), manualColumn("Signs and symptoms", columnStatus(evaluation?.clinical?.met, clinicalStarted), anyClinical(input, true)), manualColumn("Specific laboratory evidence", columnStatus(evaluation?.laboratoryEvidence?.met, labStarted), labSelector(state, input, errors, algorithm))], "PNU2");
  return `<p class="pneu-or-intro">Meeting either complete PNU2 row qualifies PNU2.</p>${criterionPanel("common", "Common bacterial or filamentous fungal pathogens", table("common"), PNU2_SOURCES.common, evaluation?.met && metAlgorithm === "common")}<div class="pneu-or">OR</div>${criterionPanel("definitive", "Viral, Legionella, and other bacterial pathogens", table("definitive"), PNU2_SOURCES.other, evaluation?.met && metAlgorithm === "definitive")}`;
}
...
function pnu3Criteria(state, input, evaluation, errors) {
  const imageStarted = input.imagingStudies.some(study => study.date || study.findings.length); const signsStarted = input.hostEvidence.length > 0 || input.measurements.length > 0 || input.clinicalFindings.length > 0; const labStarted = input.microbiologyResults.length > 0;
  return manualTable([manualColumn("Imaging evidence", columnStatus(evaluation?.imaging?.met, imageStarted), imagingSection(input, evaluation, errors, PNU3_SOURCES.imaging)), manualColumn("Host eligibility and signs", columnStatus(evaluation?.hostEligibility?.met && evaluation?.clinical?.met, signsStarted), `${hostFields(state, input)}${and()}${pnu3Clinical(input)}`), manualColumn("Specific laboratory evidence", columnStatus(evaluation?.laboratoryEvidence?.met, labStarted), pnu3Laboratory(state, input, errors))], "PNU3");
}
